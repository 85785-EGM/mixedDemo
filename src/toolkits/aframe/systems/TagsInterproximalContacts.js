...
const BUTTONS_NAME = ['decrease', 'increase', 'lock', 'clear']
const BUTTONS_COLORS = ['red', 'green', 'blue', 'white']
const EL_DATA_NAME = 'button-el-data-keeper'
...
function updateTag (tagsMap, elTags, tagID, params = {}) {
  if (!tagID) throw new Error('tagID illegal', tagID)
  if (!tagsMap.get(tagID)) {
    ...
    if (!getElTag(elTags, tagID)) {
      ...
      for (let i = 0; i < BUTTONS_NAME.length; i++) {
        const name = BUTTONS_NAME[i]
        const buttonEl = document.createElement('a-sphere')
        buttonEl.setAttribute('radius', '0.4')
        buttonEl.setAttribute('tag-button-id', i)
        buttonEl.setAttribute('color', BUTTONS_COLORS[i])
        // buttonEl.setAttribute('visible', 'false')
        buttonEl.setAttribute(
          'clickable',
          `eventName:tags-interproximal-contacts-button-click;
          ctrlEventName:tags-interproximal-contacts-button-click-ctrl;
          extraData:${name};
          elDataName:${EL_DATA_NAME};
          emitEl:a-scene
          `
        )
        buttonEl.setAttribute('position', `${i} -1 0`)
        tagTextEl.append(buttonEl)
      }
  ...
  if (tagsMap.get(tagID)) {
    ...
    const tagTextEl = getElTag(elTags, tagID)
    // add params data to button element, so clickable will emit with params data
    for (let i = 0; i < BUTTONS_NAME.length; i++) {
      const buttonEl = tagTextEl.querySelector(`[tag-button-id="${i}"]`)
      buttonEl[EL_DATA_NAME] = params
    }
    const {
      start,
      end,
    ...
    if (tag.buttons.join(',') !== buttons.join(',')) {
      tag.buttons.length = 0
      tag.buttons.push(...buttons)
    }
...
export default {
  ...
  init () {
    ...
    elTags.setAttribute('alias', 'injected-tags-interproximal-contact')
