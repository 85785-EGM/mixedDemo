import { MatrixCommit } from '@/utils/structs/history'
import {
  BoxGeometry,
  BufferGeometry,
  CylinderGeometry,
...
const INJECT = true // a flag for remind what is inject
...
class TransformControls extends Object3D {
  ...
  pointerDown (pointer) {
    ...
    this.prevMatrix = this.object.matrix.clone()
    ...
    if (this.axis !== null) {
      ...
      this.dragging = true
      if (this.object?.el) {
        this.object.el.addState('transform-changing')
        if (this.mode === 'rotate') {
          this.object.el.addState('rotate-changing')
        }
      }
  ...
  pointerUp (pointer) {
    ...
    if (this.dragging && this.axis !== null) {
      ...
      // inject emit start
      if (this.object?.el) {
        this.object.el.removeState('transform-changing')
        if (this.mode === 'rotate') {
          this.object.el.removeState('rotate-changing')
        }
      }
      if (this.object?.el?.id && INJECT) {
        const commit = new MatrixCommit(
          this.object.el.object3D,
          this.prevMatrix,
          this.object.el.object3D.matrix.clone()
        )
        this.el.sceneEl.emit('object-transform-update', {
          id: this.object?.el?.id,
          el: this.object?.el,
          commit
        })
      }
...
export default {
  ...
  schema: {
    ...
    mode: {
      type: 'string',
      default: 'translate',
      oneOf: ['translate', 'rotate', 'scale']
    },
    space: { type: 'string', default: 'world', oneOf: ['world', 'local'] },
  ...
  init () {
    ...
    el.setObject3D(this.attrName, controls)
    el.sceneEl.object3D.add(el.getObject3D(this.attrName))
  },
  update (prev) {
    const controls = this.controls
    const { mode, size, attach, enabled, visible, space, axis } = this.data
    controls.enabled = enabled
    controls.mode = mode
    controls.size = size
    controls.space = space

    controls.showX = ['all', 'x', 'xy'].includes(axis)
    controls.showY = ['all', 'y', 'xy'].includes(axis)
    controls.showZ = ['all', 'z'].includes(axis)

    prev.attach?.removeState('dragging')
    prev.attach?.removeState('transform-changing')
    prev.attach?.removeState('rotate-changing')
    if (attach) {
      if (attach !== prev.attach) {
        controls.attach(attach.object3D)
      }
    } else {
      controls.detach()
    }
    attach?.addState('dragging')
