...
export default {
  ...
  tick () {
    ...
    this.lastKey = key
    const result = this.clipping(
      tempPlane.clone(),
      this.el.getObject3D('mesh').geometry.toNonIndexed()
    )
    console.log(result)
  ...
  clipping (plane = new Plane(), geometry = new BufferGeometry()) {
    ...
    // 切割
    for (const t of originTriangles) {
      filterTriangles.push(...filterTriangleWithPlane(...t, plane))
    }
    // 补面
    const lines = doCast(geometry.boundsTree, plane.clone())
    ...
    if (lines === 0) return filterTriangles
    ...
    filterTriangles.push(...triangles.flatMap(({ a, b, c }) => [a, b, c]))
